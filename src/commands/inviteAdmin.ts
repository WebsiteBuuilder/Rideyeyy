...
export const adminData = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub.setName('economy').setDescription('Configure invites, milestones, lottery, and the reward shop')
  );
...
export async function handleAdminModal(
  ...
  switch (action) {
    ...
    case 'verification': {
      ...
      if (minMsg != null) data.minMessages = Math.max(0, Math.round(minMsg));
      if (maxAttempts != null) data.maxVerifyAttempts = Math.max(1, Math.round(maxAttempts));
      await services.invite.admin.updateConfig(guildId, data);
    ...
    case 'lottery': {
      ...
      if (perDaily != null) data.ticketsPerDaily = Math.max(0, Math.round(perDaily));
      if (perInvite != null) data.ticketsPerInvite = Math.max(0, Math.round(perInvite));
      if (perRide != null) data.ticketsPerRide = Math.max(0, Math.round(perRide));
      if (perEvent != null) data.ticketsPerEvent = Math.max(0, Math.round(perEvent));
    ...
    case 'milestoneadd': {
      ...
      await services.invite.admin.addMilestone(
        guildId,
        Math.round(threshold),
        Math.max(0, Math.round(rewardAmount)),
        roleId,
        label,
        rideKey,
        Math.max(0, Math.round(tickets))
      );
  ...
  switch (section) {
    case 'settings': {
      embed = brandedEmbed(COLOR.INFO)
        ...
        .addFields(
          { name: 'Rewards', value: onOff(cfg.rewardEnabled), inline: true },
          { name: 'Milestones', value: onOff(cfg.milestonesEnabled), inline: true },
          { name: 'Lottery', value: onOff(cfg.lotteryEnabled), inline: true },
          { name: 'Shop', value: onOff(cfg.shopEnabled), inline: true },
          { name: 'Anti-Alt', value: onOff(cfg.antiAltEnabled), inline: true },
          { name: 'Auto Announce', value: onOff(cfg.autoAnnounce), inline: true },
          { name: 'Weekly Reset', value: onOff(cfg.weeklyResetEnabled), inline: true },
          { name: 'Monthly Reset', value: onOff(cfg.monthlyResetEnabled), inline: true },
          ...
          { name: 'Lottery Channel', value: cfg.lotteryChannelId ? `<#${cfg.lotteryChannelId}>` : '—', inline: true }
        );
      rows.push(
        new ActionRowBuilder<ButtonBuilder>().addComponents(
          toggleBtn('settings', 'rewardEnabled', 'Rewards'),
          toggleBtn('settings', 'milestonesEnabled', 'Milestones'),
          toggleBtn('settings', 'lotteryEnabled', 'Lottery'),
          toggleBtn('settings', 'shopEnabled', 'Shop')
        ),
        new ActionRowBuilder<ButtonBuilder>().addComponents(
          toggleBtn('settings', 'antiAltEnabled', 'Anti-Alt'),
          toggleBtn('settings', 'autoAnnounce', 'Announce'),
          toggleBtn('settings', 'weeklyResetEnabled', 'Weekly Reset'),
          toggleBtn('settings', 'monthlyResetEnabled', 'Monthly Reset')
        ),
        new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    case 'verification': {
      embed = brandedEmbed(COLOR.ELECTRIC)
        ...
        .addFields(
          ...
          { name: 'Min Messages', value: `${cfg.minMessages}`, inline: true },
          { name: 'Max Verify Attempts', value: `${cfg.maxVerifyAttempts}`, inline: true },
          { name: 'Anti-Alt', value: onOff(cfg.antiAltEnabled), inline: true }
        );
      rows.push(
        new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    case 'lottery': {
      ...
      const lastLine = last
        ...
        : '—';
      embed = brandedEmbed(COLOR.JACKPOT)
        .setTitle(`🎟️ Weekly Lottery`)
        .setDescription(`${LINE}\nTickets accrue from the sources below and reset each draw.`)
        .addFields(
          { name: 'Status', value: onOff(cfg.lotteryEnabled), inline: true },
          { name: 'Current Pot', value: `${pot.totalTickets} tickets · ${pot.participants} entrants`, inline: true },
          { name: 'Prize', value: services.redemption.label(cfg.lotteryPrizeKey), inline: true },
          { name: 'Per Daily', value: `${cfg.ticketsPerDaily}`, inline: true },
          { name: 'Per Invite', value: `${cfg.ticketsPerInvite}`, inline: true },
          { name: 'Per Ride', value: `${cfg.ticketsPerRide}`, inline: true },
          { name: 'Per Event', value: `${cfg.ticketsPerEvent}`, inline: true },
          { name: 'Last Draw', value: lastLine, inline: false }
        );
      rows.push(
        new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    case 'shop': {
      ...
      const list = items.length
        ? items
            .map((it) => `\`${it.key}\` — ${it.label} · ${ICON.coin} ${it.priceRc} ${BRAND.ticker}${it.enabled ? '' : ' _(disabled)_'}`)
            .join('\n')
      ...
      rows.push(
        new ActionRowBuilder<ButtonBuilder>().addComponents(
          modalBtn('shop', 'shopadd', 'Add / Edit Item', ButtonStyle.Success),
          modalBtn('shop', 'shopremove', 'Remove Item', ButtonStyle.Danger),
    ...
    case 'milestones': {
      ...
      const list = milestones.length
        ? milestones
            .map((m) => {
              const parts: string[] = [];
              if (m.rewardAmount > 0) parts.push(`${ICON.coin} ${m.rewardAmount} ${BRAND.ticker}`);
              if (m.rewardRideKey) parts.push(services.redemption.label(m.rewardRideKey));
              if (m.rewardRoleId) parts.push(`<@&${m.rewardRoleId}>`);
              if (m.rewardTickets > 0) parts.push(`🎟️ ${m.rewardTickets}`);
    ...
    default: {
      ...
      embed = brandedEmbed(COLOR.EPIC)
        ...
        .addFields(
          ...
          { name: 'Min Age / Msgs', value: `${cfg.minAccountAgeDays}d / ${cfg.minMessages}`, inline: true },
          { name: 'Verified', value: `${s.verified}`, inline: true },
          { name: 'Pending', value: `${s.pending}`, inline: true },
          { name: 'Fake', value: `${s.fake}`, inline: true },
          { name: 'Lottery Pot', value: `${pot.totalTickets} tickets`, inline: true },
          { name: 'Prize', value: services.redemption.label(cfg.lotteryPrizeKey), inline: true }
        );
      break;
    }
  ...
  switch (modal) {
    ...
    case 'verification':
      builder = new ModalBuilder().setCustomId('invadm:modal:verification').setTitle('Verification').addComponents(
        ...
        input('minMessages', 'Min messages (0 = off)', String(cfg.minMessages)),
        input('maxAttempts', 'Max verify attempts', String(cfg.maxVerifyAttempts))
      );
      break;
    case 'channels':
      builder = new ModalBuilder().setCustomId('invadm:modal:channels').setTitle('Channels').addComponents(
        ...
        input('lottery', 'Lottery channel ID (blank = off)', cfg.lotteryChannelId ?? '')
      );
      break;
    case 'lottery':
      builder = new ModalBuilder().setCustomId('invadm:modal:lottery').setTitle('Lottery Settings').addComponents(
        input('perDaily', 'Tickets per /daily', String(cfg.ticketsPerDaily)),
        input('perInvite', 'Tickets per verified invite', String(cfg.ticketsPerInvite)),
      ...
      break;
    case 'shopadd':
      builder = new ModalBuilder().setCustomId('invadm:modal:shopadd').setTitle('Add / Edit Shop Item').addComponents(
        input('key', 'Item key (unique)', '', true),
        input('label', 'Display label', '', true),
        input('priceRc', 'Price (RC)', '', true),
        input('rewardKey', 'Reward key (blank = item key)', '')
      );
      break;
    case 'shopremove':
    ...
    case 'milestoneadd':
      builder = new ModalBuilder().setCustomId('invadm:modal:milestoneadd').setTitle('Add / Edit Milestone').addComponents(
        ...
        input('rewardAmount', 'Reward RC (0 = none)', '0'),
        input('roleId', 'Reward role ID (optional)', ''),
        input('rideKey', 'Ride reward key (optional)', ''),
        input('tickets', 'Lottery tickets (0 = none)', '0')
      );
      break;
    case 'milestoneremove':
